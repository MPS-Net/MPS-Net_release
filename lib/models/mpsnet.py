...
class TemporalEncoder(nn.Module):                 
    ...
    def forward(self, x, is_train=False): 
        ...
        y_cur_2=Variable(out_[:,:,8])   #
        y_cur_1=Variable(out_[:,:,7])   #
        y_cur_3=Variable(out_[:,:,9])   #

        y_bef_2=Variable(out_[:,:,5])   #
        y_bef_1=Variable(out_[:,:,4])   #
        y_bef_3=Variable(out_[:,:,6])   #

        y_aft_2=Variable(out_[:,:,11])   #
        y_aft_1=Variable(out_[:,:,10])   #
        y_aft_3=Variable(out_[:,:,12])   #          
...
class MPSnet(nn.Module):
    def __init__(
        ...
        self.nonlocalblock = TemporalEncoder(channel=2048)	#self.encoder = TemporalEncoder
        #print(self.nonlocalblock)
        #***************
        # check the number of parameters of a model https://discuss.pytorch.org/t/how-do-i-check-the-number-of-parameters-of-a-model/4325/25
        def count_parameters(model):
            return sum(p.numel() for p in model.parameters() if p.requires_grad)
        count = count_parameters(self.nonlocalblock)
        print('MoCA+HAFI: ', count)
        #***************
        ...
        def count_parameters(model):
            return sum(p.numel() for p in model.parameters() if p.requires_grad)
        count = count_parameters(self.regressor)
        print('regressor: ', count)
        #***************
